...
import { OrderSummaryReport } from '../../../../models/report.model';
...
@Component({
  ...
  styleUrls: ['./order-summary-report.scss', '../../shared/admin-table.scss'],
})
export class OrderSummaryReportComponent implements OnInit {
  ...
  reportData = signal<OrderSummaryReport | null>(null);
  ...
  fetchReport(): void {
    ...
    this.reportService.getOrderSummary(fromDate, toDate).subscribe({
      next: (data) => {
